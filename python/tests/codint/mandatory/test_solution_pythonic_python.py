...
def randlist(size: int, minval: int = -100, maxval: int = 100):
...
# AST STUFF
def visit_source_ast(function: Any, visit: Callable[[Any], bool]):
...
def instance_visitor(expected_type: Any) -> Callable[[Any], bool]:
    return lambda node: isinstance(node, expected_type)


def uses_subscript(function: Any):
    return visit_source_ast(function, instance_visitor(ast.Subscript))


def uses_list_comp(function: Any):
    return visit_source_ast(function, instance_visitor(ast.ListComp))


def uses_set_comp(function: Any):
    return visit_source_ast(function, instance_visitor(ast.SetComp))


def uses_dict_comp(function: Any):
    return visit_source_ast(function, instance_visitor(ast.DictComp))


def uses_ternary_if(function: Any):
    return visit_source_ast(function, instance_visitor(ast.IfExp))


def uses_unpacking(function: Any):
    return visit_source_ast(
        function,
        lambda node: isinstance(node, ast.Assign)
        and hasattr(node, "targets")
        and type(node.targets[0]) in (ast.Tuple, ast.List),
    )


def uses_global(function: Any, *var_names: str):
    return visit_source_ast(
        function, lambda node: isinstance(node, ast.Global) and all(var_name in node.names for var_name in var_names)
    )


def calls_function(function: Any, function_name: str):
    return visit_source_ast(
        function, lambda node: isinstance(node, ast.Call) and hasattr(node.func, "id") and node.func.id == function_name  # type: ignore
    )


def count_lines(function: Any):
    source = inspect.getsource(function)
...
def test_print_indices_and_elements():
    ...
    assert not uses_subscript(
        solution.print_indices_and_elements
    ), f"Used subscript (e.g. elements[i]) to access elements"
...
def assert_get_even_numbers_works():
    ...
    ), f"get_even_numbers returned the wrong list. Expected: {expected}. Got: {even_numbers}."
    assert uses_list_comp(
        solution.get_even_numbers_between
    ), "get_even_numbers should be implemented using a list comprehension."
...
def assert_get_perfect_squares_works():
    ...
    assert (
        expected == perfect_squares
    ), f"get_perfect_squares returned the wrong dict. Expected: {expected}. Got: {perfect_squares}."
    assert uses_dict_comp(
        solution.get_perfect_squares_between
    ), "get_perfect_squares should be implemented using a dict comprehension."
...
def test_filter_even():
    ...
    assert (
        expected == even
    ), f"filter_even_from returned the wrong list for input {numbers}. Expected: {expected}. Got: {even}."
    assert uses_list_comp(
        solution.filter_even_from
    ), "filter_even_from should be implemented using a list comprehension."
...
def test_get_number_or_minus_one():
    ...
    for n, expected in arg_answers:
        ...
        assert (
            expected == ans
        ), f"get_number_or_minus_one returned the wrong answer for n={n}. Expected: {expected}. Got: {ans}."
    assert uses_ternary_if(
        solution.get_number_or_minus_one
    ), f"get_number_or_minus_one should be implemented using the ternary if expression."
    line_count = count_lines(solution.get_number_or_minus_one)
    assert (
        line_count == 2
    ), f"get_number_or_minus_one should be implemented using a single line. Used: {line_count - 1}."
...
def test_transform_multiples_of_5():
    ...
    ), f"transform_multiples_of_5 returned the wrong answer for input {numbers}. Expected: {expected}. Got: {transformed}."
    assert uses_ternary_if(
        solution.transform_multiples_of_5
    ), f"transform_multiples_of_5 should be implemented using the ternary if expression."
    assert uses_list_comp(
        solution.transform_multiples_of_5
    ), f"transform_multiples_of_5 should be implemented using list comprehension."
    line_count = count_lines(solution.transform_multiples_of_5)
    assert (
        line_count == 2
    ), f"transform_multiples_of_5 should be implemented using a single line. Used: {line_count - 1}."
...
def test_str_lengths():
    ...
    assert (
        lengths == expected
    ), f"str_lengths returned the wrong answer for list {strings}. Expected: {expected}. Got: {lengths}."
...
def test_get_fibonacci_type():
    assert "generator" in solution.get_fibonacci_type(
        1
    ), f"get_fibonacci_type(1) returned the wrong type for fibonacci1(6)."
    ...
    assert not calls_function(
        solution.get_fibonacci_type, "type"
    ), f"get_fibonacci_type should not use function type. You should run it outside the function and paste the result as the returned string."
...
def test_difference_between_fibonacci1_and_fibonacci2():
    ...
    assert (
        len(answer) > 20
    ), f"difference_between_fibonacci1_and_fibonacci2 probably does not contain a good answer, since it is too short."
...
def test_my_avg():
    ...
    for i in range(2, n + 1):
        ...
        assert (
            expected == avg
        ), f'my_avg returned the wrong answer for numbers {", ".join(str(elements))}. Expected: {expected}. Got: {avg}.'
...
def test_print_out_in():
    ...
    inputs = [
        (even_pairs, [f"{even_pairs[i]} {even_pairs[n - i - 1]}" for i in range(n // 2)]),
        (odd_pairs, [f"{odd_pairs[i]} {odd_pairs[n - i]}" for i in range(n // 2)] + [f"{odd_pairs[n // 2]}"]),
    ]
...
def test_append_range():
    ...
    for i in range(random.randint(5, 10)):
        ...
        assert (
            r1 == r2
        ), f"Results are different for append_range using start={start} and end={end} in the {i}{suffix} loop"
    ...
    ), "Shouldn't modify the for loop in append_range"
    assert (
        signature.parameters["to"].default is None
    ), "The default value of argument 'to' should be None in append_range"


def test_global_var_func():
    assert (
        inspect.getsource(solution.global_var_func1)
...
def test_value_is_None():
    ...
    assert not solution.value_is_None(
        ComparableThing()
    ), "Function value_is_None didn't work for argument that is not None"

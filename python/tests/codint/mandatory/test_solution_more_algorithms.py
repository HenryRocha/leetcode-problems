...
import pytest
from src.coding_interviews.mandatory.more_algorithms.solution import Solution
...
def test_course_schedule(numCourses: int, prerequisites: List[List[int]], has_solution: bool):
    ...
    if has_solution:
        ...
        for course in order:
            assert all(
                dep in took for dep in prereq.get(course, [])
            ), f"Course {course} was taken before some of its prerequisites for numCourses {numCourses} and prerequisites {prerequisites}. Got: {order}."
            took.add(course)
    else:
        assert (
            order == []
        ), f"Answer for numCourses {numCourses} and prerequisites {prerequisites} should be an empty list. Got: {order}."
